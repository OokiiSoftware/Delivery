...
class NotificationManager {
  ...
  void _init() async {
    ...
    if (Platform.isAndroid) {
      await _fln.resolvePlatformSpecificImplementation<
          ...
          ?.createNotificationChannel(_channelPedidos);
    }
    ...
    Admin.inst.addCheckCompletedListener(_subscribToDebugTopic);
    await atualizarTopics();
    Log.d(TAG, 'init', 'OK');
  }

  Future finalize() async {
    Admin.inst.removeCheckCompletedListener(_subscribToDebugTopic);
    await _removerAllTopics();
  }

  //region Channels
  AndroidNotificationChannel _channelPedidos = AndroidNotificationChannel(
    NotificationChannels.NOVO_PEDIDO, // id
    'Novos Pedidos', // title
    'Canal usado para receber Pedidos.', // description
  );
  //endregion

  //region get set

  UserOki get user => FirebaseOki.userOki;

  String get meuID => user.dados.id;

  // Evita que o app envie notificações aos usuários no modo de depuração
  String getTopic(String _default) {
    if (Aplication.isRelease)
      return _default;
    else
      return NotificationTopics.depuracao;
  }
  ...
  Future<bool> sendPedidoTopic(Pedido item) async {
    try {
      String titulo = MyTexts.NOVO_PEDIDO;
      ...
      notificacao.action = NotificationActions.NOVO_PEDIDO;
      notificacao.channel = NotificationChannels.NOVO_PEDIDO;
      notificacao.topic = getTopic(NotificationTopics.novoPedido);

      await notificacao.enviarTopic();
      return true;
    } catch (e) {
      Log.e(TAG, 'sendPedidoTopic', e);
      return false;
    }
  }
        ...
        actions: [
          CupertinoDialogAction(
            ...
            onPressed: () async {
              ...
              await Navigate.to(context, GerenciaPage());
            },
          )
        ],
      )
  ...
  Future _onReceiveMessage(Map<String, dynamic> message) async {
    ...
    switch(notification.action) {
      case NotificationActions.NOVO_PEDIDO:
        _fcm.subscribeToTopic(NotificationTopics.novoPedido);
        break;
    }
  ...
  Future _showNotification(PushNotification notification) async {
    try {
      var android = AndroidNotificationDetails(
          notification.channel, 'OkiChannelName', 'OkiChannelDescription',
  ...
  Future show() async {
    var map = {
      'data': {
        'remetente': 'remetente',
        'action': NotificationActions.NOVO_PEDIDO,
        'timestamp': DataHora.now(),
        'channel': NotificationChannels.NOVO_PEDIDO,
      },
      'notification': {
        'title': 'Novo Pedido',
        'body': 'Jonas Ferreira',
    ...
    // var n = PushNotification.fromJson(map);
    _onReceiveMessage(map);
  }

  Future atualizarTopics() async {
    if (user.isGerente)
      await _fcm.subscribeToTopic(NotificationTopics.novoPedido);
  ...
  void _subscribToDebugTopic(bool b) async {
    if (FirebaseOki.isAdmin) {
  ...
  Future _removerAllTopics() async {
    await _fcm.unsubscribeFromTopic(NotificationTopics.novoPedido);
